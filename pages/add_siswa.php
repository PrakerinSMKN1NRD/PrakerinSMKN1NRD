...
<div align="center"><h3>Tambah Data Siswa</h3></div>
...
<table width="100%" border="0" cellpadding="3">
  <tr>
	...
	<input name="nis" type="text" id="nis" maxlength="4" style="width: 103px;" />
  ...
  <tr>
	...
	<span id="spryselect3">
        ...
        <?php
			...
			while($kelas=mysql_fetch_array($sql)){
				echo "<option value='".$kelas['id_jurusan']."'>".$kelas['nm_jurusan']."</option>";
			}
		?>
      </select> <span class="selectRequiredMsg">Mohon isi field ini!</span>	</span> </td>
  </tr>
  ...
  <tr>
	...
	<?php
		...
		switch ($note) {
			case "added":
			echo "<div id='box-success'>Data berhasil disimpan<br />Klik <a href='?page=data_siswa'>disini</a> untuk melihat</div>";
			break;
			break;
			case "failed":
			echo "<div id='box-error' style='width: 350px;'>Data gagal disimpan<br /><i>NIS</i> yang Anda masukkan sudah ada</div>";
			break;
			case "blank": default:
			echo "";
		}
...
<input type="hidden" name="tambah" value="siswa" />
...
<?php
	if (isset($_POST['tambah']) and $_POST['tambah']=="siswa") {
		$nis=$_POST['nis'];
		$nama=$_POST['nama'];
		...
		$tambah=mysql_query("insert into siswa values('$nis','$nama','$jenisklm','$kelas','$tmp_lahir','$tgl_lahir','$foto','$alamat','$no_telp','$jurusan','$email','$username','$password','$level')");
		
		if ($tambah) {
			direct("?page=add_siswa&notif=added");
		} else {
			direct("?page=add_siswa&notif=failed");
		}
	}
